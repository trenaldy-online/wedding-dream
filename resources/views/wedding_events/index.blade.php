@extends('layouts.app', ['title' => 'Acara Wedding'])
    ...
    <div class="budget-overview-grid">
        <div class="budget-hero-card">
            ...
            <h1 class="budget-title">
                Kelola Acara Wedding
            </h1>

            <p class="budget-subtitle">
                Buat acara terpisah untuk pihak CPW, pihak CPP, atau acara bersama.
                Budget dan tamu nantinya akan dikaitkan ke masing-masing acara.
            </p>
        </div>
        ...
        <div class="budget-side-card">
            ...
            <div class="budget-side-list">
                ...
                <div class="budget-side-item">
                    <span>Acara CPW</span>
                    <strong>{{ $events->where('event_side', 'cpw')->count() }}</strong>
                </div>

                <div class="budget-side-item">
                    <span>Acara CPP</span>
                    <strong>{{ $events->where('event_side', 'cpp')->count() }}</strong>
                </div>

                <div class="budget-side-item">
                    <span>Bersama</span>
                    <strong>{{ $events->where('event_side', 'both')->count() }}</strong>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="budget-workspace-grid">
        <div class="budget-form-card">
            <div class="budget-card-header">
                <div>
                    <h2>Tambah Acara</h2>
                    <p>Contoh: Resepsi Pihak CPW, Ngunduh Mantu Pihak CPP, Akad Nikah.</p>
                </div>
            </div>

            <form method="POST" action="{{ route('wedding-events.store') }}">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input
                        ...
                        name="event_name"
                        class="form-control"
                        placeholder="Contoh: Resepsi Pihak CPW"
                ...
                <div class="form-group">
                    ...
                    <select name="event_side" class="form-select">
                        <option value="both" {{ old('event_side') === 'both' ? 'selected' : '' }}>
                            Bersama
                        </option>
                        <option value="cpw" {{ old('event_side') === 'cpw' ? 'selected' : '' }}>
                            Pihak CPW
                        </option>
                        <option value="cpp" {{ old('event_side') === 'cpp' ? 'selected' : '' }}>
                            Pihak CPP
                        </option>
                    </select>
                </div>
        ...
        <div class="budget-table-card">
            <div class="budget-card-header table-header">
                <div>
                    <h2>Daftar Acara</h2>
                    <p>Semua rangkaian acara wedding.</p>
                </div>
            ...
            <div class="budget-table-wrapper">
                <table class="budget-table">
                    ...
                    <tbody>
                        @forelse ($events as $event)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                        <span class="checklist-muted">Belum diatur</span>
                                    @endif
                                </td>
                                ...
                                <td>
                                    <div class="budget-action-group">
                                        <a href="{{ route('wedding-events.edit', $event) }}" class="budget-edit-btn">
                                            Edit
                                        </a>

                                        <form
                                            method="POST"
                                            action="{{ route('wedding-events.destroy', $event) }}"
                                            onsubmit="return confirm('Hapus acara ini? Data budget dan tamu tidak hilang, tapi akan terlepas dari acara ini.')"
                                        >
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="budget-empty-state">
                                    Belum ada acara. Tambahkan acara pertama dari form sebelah kiri.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
